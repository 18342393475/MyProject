...
//用户资料
function userInfo(){
	console.log(getCookie( 'phprpcUrl'));
	post = {'uid': getCookie('uid'),'lang': getCookie('lang')};
	userClient.userInfo( post, function ( result, args, output, warning ){
		console.log(123);
		
		if( !result ){ $b('about0').innerHTML ="抱歉,暂无数据";  return false};
		eval( 'result = '+ result );
		var html = '<p class="pTitle1">用户基本资料</p>';
		...
		if( result.userInfo['u_birthday'] ){ 
			...
			html += '<a class="btn" href="javascript:void(0)" onclick="updateBirthday()">补充</a></p>';
		}
		html += '<p >'+ UI.DengLuMiMa +':<a class="btn" href="javascript:void(0)" onclick="changePassword(1)">'+ UI.XiuGaiDengLuMiMa +'</a></p>';
		html += '<p >'+ UI.TiKuanMiMa +':<a class="btn" href="javascript:void(0)" onclick="changePassword(2)">'+ UI.XiuGaiTiKuanMiMa +'</a></p>';

		
		html += '<p class="pTitle1">账户余额</p>';
		...
		html += ' <a class="btn" href="javascript:void(0)" onclick="bonus_account_type = \'dq\';menuTab3.setActiveTab(6)">申请体育场优惠</a></p>';
		...
		html += ' <a class="btn" href="javascript:void(0)" onclick="bonus_account_type = \'cp\'; menuTab3.setActiveTab(6)">申请快乐彩优惠</a></p>';
		...
		html += ' <a class="btn" href="javascript:void(0)" onclick="bonus_account_type = \'yx\';menuTab3.setActiveTab(6)">申请娱乐场优惠</a></p>';
		
		
		html += '<p class="pTitle1">用户联系信息</p>';
		...
		if(  result.coupon ){
			...
			html += '<p class="pTitle1">'+ UI[1006] +'</p>';								   
			html += '<table width="100%" cellpadding="4" border="0" cellspacing="1" bgcolor="#DFDFDF" frame="void" >';
			html += '<tr bgcolor="#FFFFFF">';
			html += '<th width="10%">'+ UI[1007] +'</th>';
			html += '<th width="10%">'+ UI[1008] +'</th>';
			html += '<th width="10%">'+ UI[1009] +'</th>';
			html += '<th width="10%">'+ UI[1010] +'</th>';
			html += '<th width="60%">'+ UI[1011] +'</th>';
			html += '</tr>';
		...
		//银行账户
		html += '<p class="pTitle1">已绑定的提款银行账户</p>';
		if( typeof( result.sameNameBankAccount ) == 'object' ){
			...
			html += '<p><a class="btn" href="./index.php?tabID=4">绑定银行账户</a></p>';
		}
