...
function JMenuTab(pWidth,pHeight,pBody){
	...
	*/
	...
	this.setActiveTab = function(pIndex){
		...
		if( pageID == 1 ){
			switch( pIndex ){
			case 8: if( getCookie('account_type') == 'dq' ){ getGongGao('dq'); }else if( getCookie('account_type') == 'cp' ){ getGongGao('cp'); } break;
			case 1: if( getCookie('account_type') == 'dq' ){ showOrderRow( 0, '' ); }else if( getCookie('account_type') == 'cp' ){ getUserOrderCP(); } break;
			case 2: if( getCookie('account_type') == 'dq' ){ userReport(false); }else if( getCookie('account_type') == 'cp' ){ userReport(false); } break;
			case 4: getPayType(); break;
			case 5: withdrawalStep1(); break;
			case 3: userBill(); break;
			case 0: userInfo(); break;
			case 7: if( getCookie('account_type') == 'dq' ){ getResults(); }else if( getCookie('account_type') == 'cp' ){ getResultCP();}break;
			case 6: getApplBonusForm(); break;
			}
		}else if( pageID == 2){

		}
		
		if(oPageArea.filters)oPageArea.filters[0].apply();
		
		if(self.activedTab != null){
			self.activedTab.className = "oTabBtn";
			if($(self.activedTab.tabPage)) $(self.activedTab.tabPage).style.display = "none";
		}
		...
		if(!self.activedTab) alert("undefined");
